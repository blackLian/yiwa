...
当前代码内可通过 `P3ProgressEstimator` 输出完成度百分比，完成度通过 probe 绑定真实能力状态；运行时已可区分 `UseCachedIcons` / `NativePartialMapping` / `FallbackGridMapping`，并输出 `IconSourceMode/LastSourceError`。

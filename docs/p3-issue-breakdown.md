...
- [x] ISSUE-03：图标坐标采集 Provider（已支持 Explorer ListView 句柄发现 + item count + per-icon 原生坐标+名称读取（LVM_GETITEMPOSITION/LVM_GETITEMTEXTW/ReadProcessMemory），并加入原生顺序归一化与名称回填，保留工作区网格回退）。

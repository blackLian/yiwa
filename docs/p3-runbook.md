...
13. 默认接线可通过 `DesktopIconProviderFactory.CreateDefault(...)` 创建（Win32ExplorerIconSource + RetryingDesktopIconSource），当前 Win32 source 会先尝试读取 Explorer ListView per-icon 原生坐标与名称（LVM_GETITEMPOSITION/LVM_GETITEMTEXTW + ReadProcessMemory），并按列优先顺序归一化；失败时回退为 SPI_GETWORKAREA 网格映射。
